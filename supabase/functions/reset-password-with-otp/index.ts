...
Deno.serve(async (req) => {
  ...
  if (!parsed.success) return jsonResponse({ error: 'Use a password with at least 8 characters.' }, 400)

  const email = parsed.data.email.toLowerCase()
  const admin = createClient(supabaseUrl, serviceRoleKey, { auth: { persistSession: false } })
  const { data: resetCode, error: lookupError } = await admin
    .from('password_reset_otps')
    .select('id, user_id, otp_hash, otp_salt, expires_at, attempts, used_at')
    .eq('email', email)
    .is('used_at', null)
  ...
  const { error: updateError } = await admin.auth.admin.updateUserById(resetCode.user_id, { password: parsed.data.password })
  if (updateError) return jsonResponse({ error: 'We could not update your password. Please try again.' }, 500)

  await admin.from('password_reset_otps').update({ used_at: new Date().toISOString() }).eq('id', resetCode.id)
